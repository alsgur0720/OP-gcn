...
class HD_Gconv(nn.Module):
    def __init__(self, in_channels, out_channels, A, adaptive=True, residual=True, att=False, CoM=21):
        ...
        self.conv_att = nn.Conv2d(5, 1, kernel_size=1)
        # self.pool1 = nn.Conv3d(out_channels, out_channels*2, (5, 1, 1), stride=(2, 1, 1), padding=(0, 0, 0))
        # self.pool1_bn = nn.BatchNorm3d(out_channels*2)
        # self.pool2 = nn.Conv3d(out_channels, out_channels*2, (2, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0))
        # self.pool2_bn = nn.BatchNorm3d(out_channels*2)
        # self.aggregate = nn.Conv3d(out_channels*2, out_channels, (1, 1, 1), stride=(1, 1, 1), padding=(0, 0, 0))
        # self.aggre_bn = nn.BatchNorm3d(out_channels)
    ...
    def forward(self, x):
        ...
        if self.att:
            ...
            f, b, l, v, c = out.size()
            out_mh = rearrange(out, 'f b l v c -> (f b) l v c')
            # out_max = out.max(dim=2, keepdim=False)[0]
            att = self.conv_att(out_mh)
            att = rearrange(att, '(f1 b1) l v c -> f1 (b1 l) v c', f1 = f, b1 = b)
            out_local = torch.sigmoid(att)
            # print("\n out_local.size() : ", out_local.size())
            # print("\n out_sum.size() : ", out_sum.size())
            # exit()
            out = out_sum * out_local
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        # self.l_1 = TCN_GCN_unit(3, base_channels, A, residual=False, adaptive=adaptive, att=False, CoM=CoM)
        # self.l_2 = TCN_GCN_unit(base_channels, base_channels, A, adaptive=adaptive, CoM=CoM)
        # self.l_3 = TCN_GCN_unit(base_channels, base_channels, A, adaptive=adaptive, CoM=CoM)
        # self.l_4 = TCN_GCN_unit(base_channels, base_channels, A, adaptive=adaptive, CoM=CoM)
        # self.l_5 = TCN_GCN_unit(base_channels, base_channels*2, A, stride=2, adaptive=adaptive, CoM=CoM)
        # self.l_6 = TCN_GCN_unit(base_channels*2, base_channels*2, A, adaptive=adaptive, CoM=CoM)
        # self.l_7 = TCN_GCN_unit(base_channels*2, base_channels*2, A, adaptive=adaptive, CoM=CoM)
        # self.l_8 = TCN_GCN_unit(base_channels*2, base_channels*4, A, stride=2, adaptive=adaptive, CoM=CoM)
        # self.l_9 = TCN_GCN_unit(base_channels*4, base_channels*4, A, adaptive=adaptive, CoM=CoM)
        # self.l_10 = TCN_GCN_unit(base_channels*4, base_channels*4, A, adaptive=adaptive, CoM=CoM)
        ...
        #     nn.Conv2d(base_channels*4*2, base_channels*4*2, 3, 1, 1),
        ...
        # self.aggregate3 = nn.Sequential(
        #     nn.Conv2d(base_channels*4*2, base_channels*4, 3, 1, 1),
    ...
    def forward(self, x):
        ...
        # x_flow = rearrange(x_flow, 'n c t v m -> n (m v c) t')
        # x_flow = self.data_bn(x_flow)
        # x_flow = rearrange(x_flow, 'n (m v c) t -> (n m) c t v', m=M, v=V)

        # x_flow = self.l_1(x_flow)
        # x_flow = self.l_2(x_flow)
        # x_flow = self.l_3(x_flow)
        # x_flow = self.l_4(x_flow)
        # x_flow = self.l_5(x_flow)
        # x_flow = self.l_6(x_flow)
        # x_flow = self.l_7(x_flow)
        # x_flow = self.l_8(x_flow)
        # x_flow = self.l_9(x_flow)
        # x_flow = self.l_10(x_flow)
        ...
        # multi = torch.cat((x, x_flow), dim = 1)
        
        # x = self.aggregate1(multi)
        # x = self.aggregate2(x)
        # x = self.aggregate3(x)
        # N*M,C,T,V
        c_new = x.size(1)
